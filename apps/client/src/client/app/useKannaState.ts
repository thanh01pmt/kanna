...
import type { ChatSnapshot, LocalProjectsSnapshot, SidebarChatRow, SidebarData } from "@kanna/shared/types"
...
import type { EditorOpenSettings, OpenExternalAction } from "@kanna/shared/protocol"
...
export function useKannaState(activeChatId: string | null): KannaState {
  ...
  const availableProviders = (activeChatSnapshot?.availableProviders ?? PROVIDERS).filter((provider) => !provider.disabled)
